...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>青鸟租房 -发布房屋信息</TITLE>
  ...
  <script language="JavaScript">
    ...
    function loadStreet(did){
      //发送异步请求获取区域下的街道
      $.post("getStreetByDid",{"dis_id":did},function (data) {
        //清空原有数据
      ...
      <FORM id=add_action method=post name=ss
        ...
        <DIV class=infos>
          <TABLE class=field>
            ...
            <TR>
              <TD class=field>图片：<img WIDTH="80" HEIGHT="100" src="http://localhost:80/${house.path}?id=<%=Math.random()%>"> </TD>
...
<script language="JavaScript" >
    $(function () {
        ...
        function loadstreet(distail) {
            $.post("getStreetDetail",{"dis_id":distail},function (data) {
                $("#street_id>option:gt(0)").remove();
                for(var i=0;i<data.length;i++){
                    var node=$("<option value="+data[i].id+">"+data[i].name+"</option>");
                    $("#street_id").append(node);
...
<DL class=clearfix>
  ...
  <DD class=past>填写房屋信息</DD></DL>
<DIV class=box>
<FORM method=post action="FinalUPdate" enctype="multipart/form-data">
<DIV class=infos>
<TABLE class=field>
  <TBODY>
  <TR>
  ...
  <TR>
    <TD class=field>户　　型：</TD>
    <TD><SELECT class=text name=typeId id="typeId"></SELECT>
    </TD></TR>
  <TR>
    <TD class=field>面　　积：</TD>
...
name=floorage value="${house.floorage}"></TD></TR>
  ...
  <TR>
      ...
      <TD><INPUT id=cpath class=text type=file name=cpath>
      <img src="http://localhost:80/${house.path}?a=<%=Math.random()%>" width="100" height="100">
          <input type="hidden" value="${house.path}"  name="oldpic">
      </TD>
  </TR>
